...
def summary_match_fn(truth_summary, model_summary, threshold=0.75):
    """Check semantic similarity of summaries using embeddings."""
    if pd.isna(truth_summary) or pd.isna(model_summary):
        return False, 0.0
    if str(truth_summary).strip() == "" or str(model_summary).strip() == "":
        return False, 0.0

    truth_emb = embedding_model.encode(str(truth_summary), convert_to_tensor=True)
    model_emb = embedding_model.encode(str(model_summary), convert_to_tensor=True)

    sim = util.cos_sim(truth_emb, model_emb).item()  # get float value
...
def validate_model(model_path, ground_df):
    ...
    for _, row in merged.iterrows():
        ...
        # Summary similarity
        summary_ok, summary_score = summary_match_fn(row.get("summary_truth", ""), row.get("summary_model", ""))
    ...
    # --- Only score author + date (still as per your last choice) ---
    total = len(result_df)
